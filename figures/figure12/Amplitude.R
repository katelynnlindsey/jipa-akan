#Amplitude visualisation in R v3.2.1

#To begin, open each tract in Praat (Boersma and Weenink 2013), select each 'Sound' file and save each one separately as a text file: Main menu --'Save' --'Save #as text file...'. Save these files in the same working directory you will use for R.

#Loading 'Sound' text files. Be sure to change address to the correct working directory.
nasal=read.table("%DIRECTORY%/SOUNDFILENAME_ch1.Sound", header=T, sep="\t", encoding="UTF-8")
oral=read.table("%DIRECTORY%/SOUNDFILENAME_ch1.Sound_ch2.Sound", header=T, sep="\t", encoding="UTF-8")

#For Windows users using characters other than standard ASCII you may have to change the encoding from UTF-16 to UTF-8.
#In EditPad Lite (Goyvaets, 2015) (free download) click  'Convert' in the main menu --> Text Encoding --> Under 'New Encoding' choose 'Unicode, UTF-8' --> OK. Then save.
...
#Subsetting the 'nasal' and 'oral' objects to get the amplitude (Pascal) data.
N=subset(nasal, grepl("z\\s\\[.*\\s=\\s(-|)\\d.",nasal$Nasal))
O=subset(oral, grepl("z\\s\\[.*\\s=\\s(-|)\\d.",oral$Oral))

#Removing non-numeric data from the 'N' and 'O' objects. 
N=as.data.frame(apply(N,2,function(i)gsub('\\s+z\\s\\[.*\\s=\\s', '',i)))
N=as.data.frame(apply(N,2,function(i)gsub('\\s', '',i)))

O=as.data.frame(apply(O,2,function(i)gsub('\\s+z\\s\\[.*\\s=\\s', '',i)))
O=as.data.frame(apply(O,2,function(i)gsub('\\s', '',i)))

#Converting the N & O data into numeric values
N1=c()
N1$Nasal=as.numeric(levels(N$Nasal)[N$Nasal])
O1=c()
O1$Oral=as.numeric(levels(O$Oral)[O$Oral])

NO2=cbind(N1$Nasal, O1$Oral)

#Renaming columns of NO1 to 'Nasal' and 'Oral' objects
colnames(NO2)[2]="Oral"
colnames(NO2)[1]="Nasal"

NO2=as.data.frame(NO2)

#Creating a new column with the length of N & O listed numerically from 1...n
NO2$Frame=(1:nrow(N))

#Ratio:
NO2$Ratio=abs(NO2$Nasal/(NO2$Nasal+NO2$Oral))
#Percentage:
...
plot(NO2$Nasal,type="l", ylim=c(min(NO2$Nasal)+(min(NO2$Nasal/4)),max(NO2$Nasal)+(max(NO2$Nasal/4))), col="red", xlab="", ylab="", xaxt='n', frame.plot=TRUE)
lines(NO2$Oral, type="l", lty=2, col="blue")
mtext(NO2$SegmentsAligned,side=1, cex=1.4, at=1:nrow(graphdata), line=0)
mtext(side = 2, text = "Pascals (Pa)", line = 3, cex=2)
